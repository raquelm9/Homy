...
function ResidentPost({
  ...
  const postComment = (event) => {
    ...
    new HttpService().createComment(postId, name, newComment).then(
      (data) => {
        console.log(data);
  ...
  const allComments = (comments) => {
    if (!commentsToDisplay) {
      ...
      return commentsToDisplay.map((comment, key) => (
        <ResidentPostComment
          username={comment.username}
          comment={comment.comment}
          key={key}
  ...
  return (
    <div className="post">
      <div className="post__header">
        <Avatar className="post__avatar" alt={username} src={userAvatarUrl} />
        <h4>{username}</h4>
      </div>
      ...
      {image ? (
        <img
          src={getImagePath()}
          className="img-fluid image-height"
          alt="images"
        />
      ) : null}
